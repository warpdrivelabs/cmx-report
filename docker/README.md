...
```bash
docker run -d --name cmx-rpt -p 8092:8092 \
...
```bash
cd backend/cmx-report/docker
mkdir -p runtime/config
docker compose run --rm cmx-rpt cat /opt/cmx/config-template.toml > runtime/config/docker.toml
